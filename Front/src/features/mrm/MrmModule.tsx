...
import { useEffect, useState } from 'react';
import { useSearchParams } from 'react-router-dom';
import { mrmApi, certApi, kpiReviewApi, auditApi,  prpApi, prsApi, actionTrackerApi, deptApi } from '../../api/qms.api';
import { exportCSV } from '../master-data/chartUtils';
import { FiAlertTriangle, FiTrash2, FiCheck, FiX as FiXIcon } from 'react-icons/fi';
import type { MrmPlan, MrmMinutes as MrmMinutesType, MrmAgenda, Certification, AuditPlan, NC, CertRef, ProcessReviewPlan as PRP, ProcessReviewSheet as PRS, ReviewChecklistItem, ActionTracker as AT, Department } from '../qms/types';
import { apiMsg } from '../qms/types';
...
function MrmKpiReview({ certId }: { certId: number | null }) {
  ...
  // KPI performance line items — kpiCode, kpiName, department, frequency, target, unit
  // are auto-fetched from /kpi-reviews/load-kpi-data (backed by KpiEntry actuals).
  // actualValue / achievementStatus / remarks are editable by the reviewer.
  const [items, setItems] = useState<Array<{
    kpiCode: string; kpiName: string; department: string; frequency: string;
    target: number; unit: string;
    actualValue: string; achievementPercent: string; achievementStatus: string; remarks: string;
  }>>([]);
  ...
  const effectiveCertId = certId ?? (plans.find(p => p.id === planId) as any)?.certification?.id ?? null;

  // Load MRM plans for the selector
  useEffect(() => {
    const planLoader = certId ? mrmApi.getPlansByCert(certId) : mrmApi.getPlans();
    planLoader
      .then((d: unknown) => setPlans(Array.isArray(d) ? d as MrmPlan[] : []))
      .catch(() => setPlans([]))
      .finally(() => setLoading(false));
  }, [certId]);

  // When a plan is picked: check for an existing KPI Review first (edit mode).
  // If none exists yet, auto-fetch actual KPI performance data to prefill a new review.
  useEffect(() => {
    if (!planId) {
      setReviewId(null); setItems([]); setReviewStatus('DRAFT');
      setReviewDecision(''); setManagementComments(''); setResponsiblePerson('');
      setTargetCompletionDate(''); setReviewedBy('');
      return;
    }
    setItemsLoading(true);
    (kpiReviewApi.getByMrm(planId) as Promise<any>)
      .then((res: any) => {
        const existing = res && res.id ? res : null;
        if (existing) {
          // Edit mode — use the saved review as-is
          setReviewId(existing.id);
          setReviewStatus(existing.reviewStatus || 'DRAFT');
          setReviewDecision(existing.reviewDecision || '');
          setManagementComments(existing.managementComments || '');
          setResponsiblePerson(existing.responsiblePerson || '');
          setTargetCompletionDate(existing.targetCompletionDate || '');
          setReviewedBy(existing.reviewedBy || '');
          setItems((existing.kpiPerformanceItems || []).map((it: any) => ({
            kpiCode: it.kpiCode, kpiName: it.kpiName, department: it.department || '',
            frequency: it.frequency || '', target: it.target ?? 0, unit: it.unit || '',
            actualValue: it.actualValue ?? '', achievementPercent: it.achievementPercent ?? '',
            achievementStatus: it.achievementStatus || '', remarks: it.remarks || '',
          })));
          setItemsLoading(false);
        } else if (effectiveCertId) {
          // New review — pull real actual values from KpiEntry via load-kpi-data
          setReviewId(null); setReviewStatus('DRAFT');
          setReviewDecision(''); setManagementComments(''); setResponsiblePerson('');
          setTargetCompletionDate(''); setReviewedBy('');
          kpiReviewApi.loadKpiData(effectiveCertId, year)
            .then((d: unknown) => setItems(Array.isArray(d) ? d as any[] : []))
            .catch(() => setItems([]))
      ...
      .catch(() => { setReviewId(null); setItems([]); setItemsLoading(false); });
  }, [planId, year, effectiveCertId]);

  const setItemField = (kpiCode: string, field: 'actualValue' | 'achievementStatus' | 'remarks', value: string) => {
    setItems(prev => prev.map(it => {
      if (it.kpiCode !== kpiCode) return it;
      const updated = { ...it, [field]: value };
      if (field === 'actualValue') {
        const val = value !== '' ? Number(value) : null;
        if (val != null && it.target > 0) {
          const pct = Math.round((val / it.target) * 100 * 100) / 100;
          updated.achievementPercent = String(pct);
          if (pct >= 100) updated.achievementStatus = 'ACHIEVED';
          else if (pct >= 75) updated.achievementStatus = 'PARTIALLY_ACHIEVED';
          else updated.achievementStatus = 'NOT_ACHIEVED';
        } else {
          updated.achievementPercent = '';
          updated.achievementStatus = '';
        }
      }
      return updated;
    }));
  ...
  const stats = {
    total:       items.length,
    achieved:    items.filter(i => i.achievementStatus === 'ACHIEVED').length,
    partial:     items.filter(i => i.achievementStatus === 'PARTIALLY_ACHIEVED').length,
    notAchieved: items.filter(i => i.achievementStatus === 'NOT_ACHIEVED').length,
  };

  const saveReview = async () => {
    if (!planId) { alert('Please select an MRM Plan before saving the KPI review.'); return; }
    if (!reviewDecision) { alert('Please select a Review Decision before saving.'); return; }
    setSaving(true);
    try {
      const kpiPerformanceItems = items.map(it => ({
        kpiCode: it.kpiCode,
        kpiName: it.kpiName,
        ...
        target: it.target,
        unit: it.unit,
        actualValue: it.actualValue !== '' ? Number(it.actualValue) : null,
        achievementPercent: it.achievementPercent !== '' ? Number(it.achievementPercent) : null,
        achievementStatus: it.achievementStatus || null,
        remarks: it.remarks || '',
      }));

      const payload: any = {
        mrmPlan: { id: planId },
        certification: effectiveCertId ? { id: effectiveCertId } : null,
        kpiPerformanceItems,
        reviewDecision,
        managementComments,
        responsiblePerson,
      ...
      const saved: any = reviewId
        ? await kpiReviewApi.update(reviewId, payload)
        : await kpiReviewApi.create(payload);

      if (saved && saved.id) { setReviewId(saved.id); setReviewStatus(saved.reviewStatus || 'DRAFT'); }
      alert(reviewId ? 'KPI Review updated successfully.' : 'KPI Review saved as Draft. Open it again to mark it Reviewed/Approved.');
    } catch (e: unknown) {
      alert(apiMsg(e, 'Failed to save KPI review'));
    } finally {
      setSaving(false);
    }
  };
  ...
  const printKpiReview = () => {
    ...
    w.document.write(`
      ...
      <h1>KPI Review — Management Review Meeting</h1>
      <p>Generated: ${new Date().toLocaleDateString('en-IN')} | Total KPIs: ${items.length} | Decision: ${reviewDecision || '—'}</p>
      <table>
        <thead><tr><th>KPI Code</th><th>KPI Name</th><th>Target</th><th>Unit</th><th>Actual</th><th>Achievement %</th><th>Status</th><th>Remarks</th></tr></thead>
        <tbody>${items.map(m=>`<tr><td><b>${m.kpiCode}</b></td><td>${m.kpiName||'—'}</td><td><b>${m.target??'—'}</b></td><td>${m.unit}</td><td>${m.actualValue??'—'}</td><td>${m.achievementPercent??'—'}</td><td>${m.achievementStatus||'—'}</td><td>${m.remarks||'—'}</td></tr>`).join('')}</tbody>
      </table>
      </body></html>
    `);
    w.document.close(); w.print();
  ...
  return (
    <div className="space-y-4">
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 px-5 py-3.5 flex items-center justify-between flex-wrap gap-3">
        <div>
          <h3 className="text-sm font-semibold text-gray-700">KPI Review — Auto Fetched Actuals from KPI Module</h3>
          <p className="text-xs text-gray-500 mt-0.5">Review KPI performance and record the management decision for this MRM</p>
        </div>
        <div className="flex items-center gap-2">
          <select value={planId} onChange={e => setPlanId(e.target.value ? Number(e.target.value) : '')}
            ...
            <option value="">Select MRM Plan…</option>
            {plans.map(p => (
              <option key={p.id} value={p.id}>{p.mrmRefNo} — {p.meetingDate || 'no date'}</option>
            ))}
          </select>
          <input type="number" value={year} onChange={e => setYear(Number(e.target.value))}
            className={`${inputCls} w-24`} title="KPI data year" />
          {items.length > 0 && (
            <button onClick={printKpiReview} className="flex items-center gap-1.5 px-3 py-2 rounded-lg border border-red-300 text-red-700 text-xs hover:bg-red-50">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { label: 'Total KPIs',        value: stats.total,       color: BRAND     },
          { label: 'Achieved',          value: stats.achieved,    color: '#10b981' },
          { label: 'Partially Achieved',value: stats.partial,     color: '#f59e0b' },
          { label: 'Not Achieved',      value: stats.notAchieved, color: '#ef4444' },
        ].map(k => (
          <div key={k.label} className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 text-center">
            <p className="text-2xl font-bold" style={{ color: k.color }}>{k.value}</p>
            <p className="text-xs text-gray-500 mt-0.5">{k.label}</p>
      ...
      {!planId && (
        <div className="bg-amber-50 border border-amber-200 text-amber-800 text-xs rounded-xl px-4 py-3 flex items-center gap-2">
          <i className="fas fa-circle-info" /> Select an MRM Plan above to load KPI actuals and save a review against it.
        </div>
      )}

      {planId && itemsLoading ? (
        <div className="py-10 text-center"><i className="fas fa-spinner fa-spin text-xl" style={{ color: BRAND }} /></div>
      ) : planId && items.length === 0 ? (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 py-12 text-center text-gray-400">
          <i className="fas fa-chart-line text-3xl mb-2 block opacity-20" />
          No KPI masters/entries found for this certification & year.
        </div>
      ) : items.length > 0 ? (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 text-gray-500 text-xs uppercase">
                {['KPI Code', 'KPI Name', 'Target', 'Unit', 'Actual', 'Achv %', 'Status', 'Remarks'].map(h => (
                  <th key={h} className="px-4 py-3 text-left whitespace-nowrap">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {items.map((row, i) => (
                <tr key={row.kpiCode || i} className="hover:bg-gray-50">
                  <td className="px-4 py-3 font-mono text-xs font-bold" style={{ color: BRAND }}>{row.kpiCode}</td>
                  <td className="px-4 py-3 font-medium text-gray-800">{row.kpiName || '—'}</td>
                  <td className="px-4 py-3 font-bold">{row.target ?? '—'}</td>
                  <td className="px-4 py-3 text-gray-500 text-xs">{row.unit}</td>
                  <td className="px-4 py-3">
                    <input type="number" value={row.actualValue}
                      onChange={e => setItemField(row.kpiCode, 'actualValue', e.target.value)}
                      className="w-20 border border-gray-200 rounded px-2 py-1 text-xs focus:outline-none" />
                  </td>
                  <td className="px-4 py-3 text-gray-500 text-xs">{row.achievementPercent !== '' ? `${row.achievementPercent}%` : '—'}</td>
                  <td className="px-4 py-3">
                    <select value={row.achievementStatus}
                      onChange={e => setItemField(row.kpiCode, 'achievementStatus', e.target.value)}
                  ...
                  <td className="px-4 py-3">
                    <input value={row.remarks}
                      onChange={e => setItemField(row.kpiCode, 'remarks', e.target.value)}
      ...
      {planId && items.length > 0 && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5 space-y-3">
          <h4 className="text-sm font-semibold text-gray-700">Overall Management Review Decision</h4>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              ...
              <select value={reviewDecision} onChange={e => setReviewDecision(e.target.value)} className={inputCls}>
                ...
                <option value="CORRECTIVE_ACTION_REQUIRED">Corrective Action Required</option>
                <option value="MONITOR">Monitor</option>
                <option value="ESCALATE">Escalate</option>
              </select>
              {reviewDecision === 'CORRECTIVE_ACTION_REQUIRED' && (
                <p className="text-[11px] text-amber-600 mt-1">This will auto-create an Action Tracker item on save.</p>
              )}
            </div>
            <div>
              <label className={labelCls}>Responsible Person</label>
              <input value={responsiblePerson} onChange={e => setResponsiblePerson(e.target.value)} className={inputCls} />
            </div>
            <div>
              <label className={labelCls}>Target Completion Date</label>
              <input type="date" value={targetCompletionDate} onChange={e => setTargetCompletionDate(e.target.value)} className={inputCls} />
            </div>
            <div>
              <label className={labelCls}>Reviewed By</label>
              <input value={reviewedBy} onChange={e => setReviewedBy(e.target.value)} className={inputCls} />
            </div>
            {reviewId && (
              <div>
            ...
            <div className="md:col-span-2">
              ...
              <textarea value={managementComments} onChange={e => setManagementComments(e.target.value)} rows={2} className={inputCls} />
            </div>
          </div>
          <div className="flex justify-end">
            <button onClick={saveReview} disabled={saving}
              ...
              {saving ? <><i className="fas fa-spinner fa-spin mr-1" />Saving...</> : reviewId ? 'Update KPI Review' : 'Save KPI Review'}
            </button>
          </div>
        </div>
      )}
