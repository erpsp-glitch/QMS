...
//   // Backend returns { success, message, data } (see ApiResponse.ok on the server side).
//   // Handle both a raw array and a wrapped {data: [...]} shape, so we don't
//   // silently fall back to an empty list when the shape doesn't match.
...
//     Promise.allSettled([employeeApi.getAll(), deptApi.getAll()])
//       .then(([eRes, dRes]) => {
//         setRows(eRes.status === "fulfilled" ? extractList<Employee>(eRes.value) : []);
//         setDepts(dRes.status === "fulfilled" ? extractList<Department>(dRes.value) : []);
//         if (eRes.status === "rejected") {
//           console.error("Failed to load employees:", eRes.reason);
//         }
...
//     // THIS was missing before — the form built a payload but never sent it
//     // to the backend, so nothing was actually created/updated from this button.
//     if (editing) {
...
export default function EmployeePage() {
  ...
  // Backend controller returns ApiResponse.ok(list), which serializes as:
  //   { success: true, message: "...", data: [ ...employees ] }
  // Depending on how your axios wrapper (qms.api.ts) is set up, employeeApi.getAll()
  // might resolve to:
  //   (a) the raw array directly                -> [ {...}, {...} ]
  //   (b) the ApiResponse object                 -> { success, message, data: [...] }
  //   (c) a full axios response (not unwrapped)   -> { data: { success, message, data: [...] } }
  //   (d) a Spring Page object (if pagination is added later) -> { content: [...] }
  // This function checks all of them so we never silently fall back to an empty list.
  const extractList = <T,>(value: any, label: string): T[] => {
    if (Array.isArray(value)) return value;
    if (value && Array.isArray(value.data)) return value.data;
    if (value && value.data && Array.isArray(value.data.data)) return value.data.data;
    if (value && Array.isArray(value.content)) return value.content; // Spring Page shape
    if (value && Array.isArray(value.result)) return value.result;
    if (value && Array.isArray(value.payload)) return value.payload;

    // Nothing matched — log the raw shape so we can see exactly what the API returned.
    console.warn(`[${label}] Unexpected API response shape, showing empty list. Raw value:`, value);
    return [];
  };
  ...
  const load = () => {
    ...
    Promise.allSettled([employeeApi.getAll(), deptApi.getAll()])
      .then(([eRes, dRes]) => {
        setRows(eRes.status === "fulfilled" ? extractList<Employee>(eRes.value, "employees") : []);
        setDepts(dRes.status === "fulfilled" ? extractList<Department>(dRes.value, "departments") : []);
        if (eRes.status === "rejected") {
          console.error("Failed to load employees:", eRes.reason);
        }
        if (dRes.status === "rejected") {
